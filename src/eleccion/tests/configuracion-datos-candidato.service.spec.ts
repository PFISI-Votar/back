...
describe('ConfiguracionDatosCandidatoService', () => {
  ...
  beforeEach(async () => {
    mockCandidatoRepository.createQueryBuilder.mockReturnValue(mockQueryBuilder);
  ...
  it('debe crear configuración vacía por defecto', async () => {
    const config = { idConfiguracion: 1, idEleccion: 1, campos: [] };
  ...
  it('debe bloquear guardado si hay candidatos registrados', async () => {
    ...
    await expect(
      service.guardar(1, { campos: camposEjemplo }),
  ...
  it('debe permitir guardar configuración vacía', async () => {
    ...
    mockConfigRepository.save.mockImplementation(async (config) => config);
