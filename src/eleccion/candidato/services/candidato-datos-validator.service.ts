...
export class CandidatoDatosValidatorService {
  ...
  private validateCampo(
    ...
    if (isEmpty) {
      if (campo.obligatorio) {
        return [
          { clave: campo.clave, message: `${campo.etiqueta} es obligatorio` },
        ];
      }
      return [];
    }
  ...
  private validateTexto(
    ...
    if (typeof valor !== 'string') {
      return [
        { clave: campo.clave, message: `${campo.etiqueta} debe ser texto` },
      ];
    }
    const texto = sanitizeHtml(valor.trim(), {
      allowedTags: [],
      allowedAttributes: {},
    });
  ...
  private validateEmail(
    ...
    if (typeof valor !== 'string') {
      return [
        { clave: campo.clave, message: `${campo.etiqueta} debe ser un email` },
      ];
    }
    ...
    if (!EMAIL_REGEX.test(email)) {
      return [
        {
          clave: campo.clave,
          message: `${campo.etiqueta} no es un email válido`,
        },
      ];
    }
  ...
  private validateUrl(
    ...
    if (typeof valor !== 'string') {
      return [
        { clave: campo.clave, message: `${campo.etiqueta} debe ser una URL` },
      ];
    }
    ...
    try {
      ...
      if (!['http:', 'https:'].includes(parsed.protocol)) {
        return [
          {
            clave: campo.clave,
            message: `${campo.etiqueta} debe usar http o https`,
          },
        ];
      }
    } catch {
      return [
        {
          clave: campo.clave,
          message: `${campo.etiqueta} no es una URL válida`,
        },
      ];
    }
  ...
  private validateNumero(
    ...
    if (!Number.isFinite(numero)) {
      return [
        { clave: campo.clave, message: `${campo.etiqueta} debe ser un número` },
      ];
    }
    const errors: CampoCandidatoError[] = [];
  ...
  private validateFecha(
    ...
    if (typeof valor !== 'string') {
      return [
        {
          clave: campo.clave,
          message: `${campo.etiqueta} debe ser una fecha válida`,
        },
      ];
    }
    ...
    if (Number.isNaN(fecha.getTime())) {
      return [
        {
          clave: campo.clave,
          message: `${campo.etiqueta} no es una fecha válida`,
        },
      ];
    }
    return [];
  ...
  private validateBooleano(
    ...
    if (typeof valor !== 'boolean') {
      return [
        {
          clave: campo.clave,
          message: `${campo.etiqueta} debe ser verdadero o falso`,
        },
      ];
    }
    return [];
      ...
      try {
        ...
        if (!regex.test(texto)) {
          errors.push({
            clave: campo.clave,
            message:
              patternMessage ?? `${campo.etiqueta} tiene un formato inválido`,
          });
        }
      } catch {
        errors.push({
